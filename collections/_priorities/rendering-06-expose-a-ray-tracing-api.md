...
description: |
  Hardware ray tracing is slowly becoming more widespread. It's starting to be common for desktop computers to support hardware ray tracing, and soon after, mobile devices will support it as well. We want to expose an API for hardware ray tracing through our `RenderingDevice` so that users can begin to make use of it. Then, eventually, we want to use that API to leverage hardware ray tracing in the _Forward+_ renderer.
details:
  - type: prs
    content: |
