...
excerpt: "New plugins for Godot XR."
categories: ["progress-report"]
author: Bastiaan Olij
image: /storage/blog/covers/february-2025-xr-progress-update.webp
image_caption_description: "TBD"
date: 2025-02-15 12:00:00
---

## Announce meta toolkit extension

We are pleased to announce the release of the [Godot Meta Toolkit](https://github.com/godot-sdk-integrations), a GDExtension plugin that exposes Meta's [Platform SDK](https://developers.meta.com/horizon/documentation/native/ps-platform-intro) and provides other tools to simplify and accelerate XR development on Meta's platform.
...
You can download version 1.0.0 [from GitHub](https://github.com/godot-sdk-integrations/godot-meta-toolkit/releases/tag/1.0.0-stable) or the [asset library].
...
Support of the Platform SDK in the Godot Meta Toolkit is done using codegen, which automatically generates the Godot classes by processing the [Platform SDK’s official C headers](https://developers.meta.com/horizon/downloads/package/oculus-platform-sdk/). This approach allows to easily keep the Godot Meta Toolkit up-to-date with the latest updates to the Platform SDK, and provides Godot APIs which match the Platform SDK C, Unity and Unreal APIs.

As of version 1.0.0, the Godot Meta Toolkit supports v71 of the Platform SDK.
...
The [Meta XR Simulator](https://developers.meta.com/horizon/documentation/unity/xrsim-intro) is the counterpart of the Godot XR Editor as it allows developers to test XR applications directly on their computer (Windows or MacOS), removing the need to constantly put the headset on and off, thus leading to faster iteration.
...
In addition, the XR sample projects are also available on the GitHub release and the [Asset Library](https://godotengine.org/asset-library/asset?filter=XR+Sample&category=&godot_version=&cost=&sort=updated).
...
- Add the `xr/openxr/extensions/automatically_request_runtime_permissions` project setting to enable/disable automatic requests for runtime permissions
  ...
  - Developers can disable that behavior so that the app / game logic can itself request the permissions in a context-specific manner
...
- Add export option to enable enable / disable sharing of Meta's spatial anchors
...
This version of the XR Toolkit has been updated to contain [Godot 4.2 mesh formats](https://godotengine.org/article/godot-4-2-arrives-in-style/#critical-and-breaking-changes) and thus **requires Godot 4.2+**.
...
- The StartXR startup script has had a cleanup pass, it now:
  - properly handles the pass through system changes in Godot 4.3. Godot 4.3 saw changes in basing pass through on the environment blend mode and moved logic into the vendor plugin to improve platform support for pass through. XR Tools now makes use of this new system.
  - adds proper support for the immersive-ar and immersive-vr webXR modes
