...
Today, we’re happy to announce that GABE, the Godot Android Build Environment is now stable and available on the [Google Play Store](https://play.google.com/store/apps/details?id=org.godotengine.godot_gradle_build_environment) and [Meta Horizon Store](link).
...
This is a companion app for the Godot Editor on Android which works in the background whenever you trigger a Gradle export. GABE handles downloading and managing all the build dependencies. After you grant it access to your project for the first time, it *just works* for subsequent exports in the background.

## What GABE brings to your workflow:

- You can now generate **AAB** files (Android App Bundles) directly on your Android device. You no longer need to move your project to a desktop just to get it ready for the Play Store.
    - This also means that you can now create, develop, export and publish a Godot game to the Play Store using only an Android device.

- If your project uses plugins like Google Play Billing or AdMob, you previously had to use a PC to export. Now, you can build your apps with plugin support directly on Android.

- Since XR apps require Gradle for vendor-specific plugins and features, GABE enables you to build and deploy XR projects directly from within your headsets. (In case you missed it: Godot is officially available on Meta Quest and Android XR with an experimental build for Pico 4 also available).

To sum it all up, with this release we are removing one of the major limitations of the Android editor, the [ability to generate Android binaries on device](https://godotengine.org/article/android_godot_editor_play_store_beta_release/#generating-android-apk-binaries). You now get the same export customizations and features set on your phone or tablet that you’d expect on a desktop.

## Download

You can download the GABE from:
...
It’s been a long road to bring full build support to the Android editor, and we are excited to see what you build *entirely* on Android!
