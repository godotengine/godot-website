...
Since the [last blog post](https://godotengine.org/article/multiplayer-changes-godot-4-0-report-4), we rewrote the scene replication API to be more user-friendly, adding two "configuration" nodes:

- The `MultiplayerSpawner` node to configure where and who can instantiate nodes remotely.
- The `MultiplayerSynchornizer` node to configure which node properties can be synchronized by which peer.
...
So, now that we have our scene set up and peers can connect, let's add a MultiplayerSynchronizer to our object scene and configure it to sync some of its properties.
...
The MultiplayerSpawner node automates the process of replicating dynamically instantiated nodes across peers including when joining mid-game or reconnecting.
...
Currently, using `get_tree().change_scene()` during a multiplayer session can be problematic when players join mid-game or re-join a match, while we are working on supporting `change_scene` out of the box in future Godot releases, it is currently possible to simulate the behavior using a MultiplayerSpawner to spawn the level scene.
...
Now let's add a `change_level` function which instantiate the (selected) level and call it from the `start_game` function.
...
We can now run the game and test that connecting to a host will correctly spawn the appropriate level and players, giving us control of our character, and properly synchronizing the object positions.

![Client and server](/storage/blog/multiplayer-in-godot-4-0-scene-replication/client_server_side_by_side.png)

Here is the full project that you can [download](/storage/blog/multiplayer-in-godot-4-0-scene-replication/project.zip)
